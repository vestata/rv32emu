...
#define VI_LOOP(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)           \
    ...
    for (uint8_t k = 0; k < itr; k++) {                                      \
        if (ir->vm) {                                                        \
            ans = ((op_##op((tmp_1 >> (k << (SHIFT))), (op2))) & (MASK_BIT)) \
                  << (k << (SHIFT));                                         \
        } else {                                                             \
            ans = (vm & (0x1 << k))                                          \
                      ? ((op_##op((tmp_1 >> (k << (SHIFT))), (op2))) &       \
                         (MASK_BIT))                                         \
                            << (k << (SHIFT))                                \
                      : (tmp_d & (MASK_BIT << (k << (SHIFT))));              \
        }                                                                    \
        rv->V[des + j][i] += ans;                                            \
    }

#define VI_LOOP_LEFT(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)      \
    uint32_t tmp_1 = rv->V[op1 + j][i];                                      \
    uint32_t tmp_d = rv->V[des + j][i];                                      \
    if (rv->csr_vl % itr) {                                                  \
...
#define VV_LOOP(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)            \
    uint32_t tmp_1 = rv->V[op1 + j][i];                                       \
...
#define VV_LOOP_LEFT(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)       \
    uint32_t tmp_1 = rv->V[op1 + j][i];                                       \
    uint32_t tmp_2 = rv->V[op2 + j][i];                                       \
    uint32_t tmp_d = rv->V[des + j][i];                                       \
    if (rv->csr_vl % itr) {                                                   \
...
#define VX_LOOP(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)             \
    uint32_t tmp_1 = rv->V[op1 + j][i];                                        \
    ...
    for (uint8_t k = 0; k < itr; k++) {                                        \
        if (ir->vm) {                                                          \
            ans = ((op_##op((tmp_1 >> (k << (SHIFT))), (tmp_2))) & (MASK_BIT)) \
                  << (k << (SHIFT));                                           \
        } else {                                                               \
            ans = (vm & (0x1 << k))                                            \
...
#define VX_LOOP_LEFT(des, op1, op2, op, SHIFT, MASK_BIT, i, j, itr, vm)        \
    uint32_t tmp_1 = rv->V[op1 + j][i];                                        \
    uint32_t tmp_2 = rv->X[op2];                                               \
    uint32_t tmp_d = rv->V[des + j][i];                                        \
    if (rv->csr_vl % itr) {                                                    \
